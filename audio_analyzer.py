...
def record() -> list[list[float]]:
    ...
    return np.squeeze(recorded_data, axis=2) # make the list of lists contain floats instead off [floats] 
...
def time_analysis(recorded_data: list[list[float]]) -> tuple[list, list]:
...
def freq_analysis(recorded_data: list[list[float]]) -> tuple[list, list]:
